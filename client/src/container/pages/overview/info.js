...
const Info = ({ formData, setformData, imageURL, setImageURL }) => {
  ...
  useEffect(() => {
    ...
    var currentUser = decrypt(enc_user_detail);
    ...
    // } else
    if (currentUser.sessdata.store[0].user_role == 'super_admin') {
        ...
        <Row gutter={30}>
          <Col md={12} xs={24}>
            ...
            >
              <Select
                style={{ width: '100%' }}
                onChange={seletedValue => setformData({ ...formData, active: seletedValue })}
              >
